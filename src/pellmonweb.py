...
class PellMonWebb:
    ...
    def image(self, timeChoice='time1h', direction='', **args):
        ...
        fd=NamedTemporaryFile(suffix='.svg')
        graph_file=fd.name
        RrdGraphString1="rrdtool graph "+graph_file+" --imgformat SVG --lower-limit 0 --right-axis 1:0 --full-size-mode --width "+graphWidth+" --height 400 --end now-"+graphTimeEnd+"s --start now-"+graphTimeStart+"s "
        ...
        return serve_fileobj(fd, content_type='image/svg+xml')
    ...
    def consumption(self, **args):
        ...
        if consumption_graph:
            ...
            RrdGraphString1="rrdtool graph "+consumption_file+" --imgformat SVG --full-size-mode --width "+graphWidth+" --right-axis 1:0 --right-axis-format %%1.1lf --height 400 --end %u --start %u-86400s "%(now,now)
